...
const features = [
  {
    ...
    sdd: ["Frontend SDD §12：Learning Answer 使用結構化卡片，而非只輸出 Markdown。", "Backend SDD §7：Teaching Service 依年級檢索教材、生成回答並建立 Learning Gap Insight。"],
    objectives: ["找到六個理化主題入口", "讀懂回答步驟與教材來源", "操作動畫參數並觀察結果", "完成理解檢核與追問"],
    prerequisites: ["以學生角色進入 Demo", "建議從牛頓力學情境開始", "預留約 3 分鐘完成完整體驗", "offline_demo 每位示範學生每日 20 次 Agent 額度"],
    ...
    boundaries: ["目前 Agent 明確標示為 Fake Demo／offline_demo。", "支援六個理化主題與各三個既有追問；不支援內容回 503 且不扣用量。", "JPEG／PNG 可上傳，但離線模式不辨識圖片內容，會清楚告知只依文字回答。"],
    ...
    trouble: [["頁面顯示不支援", "改用六個理化主題範圍內的問題。"], ["圖片沒有被解題", "offline_demo 不做 OCR；請補上題目文字。"], ["額度已用完", "依 Retry-After／隔日再試；重新登入不會重置。"]],
  },
  {
    id: "resources",
    no: "02",
    ...
    sdd: ["Frontend SDD §§14–17：快速分類入口與個人化推薦必須清楚區分。", "Backend SDD §§8、20–23：Embedding 只找候選方案；資格採保守判斷，敏感記憶必須先取得同意。"],
  ...
  },
  {
    ...
    sdd: ["Frontend SDD §19：所有個人化通知都必須回答「為什麼我會收到？」。", "Backend SDD §§41–43：Alert Catalog 依地區與結構化 Profile 做簡單、可解釋的匹配。"],
    ...
    boundaries: ["Demo 通知來自本機 data/alerts，不是即時政府 API。", "通知提供資訊與入口，不代表替使用者完成申請或預約。", "個人化理由必須可理解，不應只顯示『系統推薦』。"],
    ...
    trouble: [["沒有看到農業提醒", "確認學生 Profile 地區與家庭特徵是否符合示範規則。"], ["已經是已讀", "可還原通知，或用未讀的另一則示範。"], ["期待即時政府公告", "目前為本機示範 Catalog；文件會明確標示。"]],
  },
  {
    id: "teacher",
    no: "04",
    ...
    sdd: ["Frontend SDD §§21–24：教師端以 KPI、Learning Gap 與需要關注的學習訊號為核心。", "Backend SDD §§34–38：每則訊息最多一個 Primary Insight，教師 API 回傳授權學習摘要。"],
  ...
  },
  {
    ...
    sdd: ["Frontend SDD §§25–30：政府工作台是 Pitch 核心，但不得顯示姓名、完整對話或家庭個資。", "Backend SDD §§39–46：Government API 只能讀 Insight Events，不得讀 Raw Messages。"],
...
function renderFeature(feature, featureIndex) {
  ...
  <section class="manual-page cover-page observe" id="cover">${pageHeader(feature, 1, "功能總覽")}<div class="cover-grid"><div class="cover-copy"><span class="eyebrow">${esc(feature.role)} · JUDGE GUIDE</span><h1>${esc(feature.title)}</h1><p class="lead">${esc(feature.short)}</p><blockquote>${esc(feature.vision)}</blockquote><div class="cover-actions screen-only"><a href="${feature.entry}" target="_blank" rel="noopener">開啟實際 Demo</a><button type="button" data-complete-guide>完成本教學</button></div><div class="route-chip"><span>體驗入口</span><code>http://localhost:8080${esc(feature.route)}</code></div></div>${heroImage(feature)}</div></section>

  <section class="manual-page observe" id="why">${pageHeader(feature, 2, "願景與問題")}<div class="section-title"><span>WHY IT MATTERS</span><h2>這個功能解決什麼問題？</h2><p>從使用者阻力出發，再看它在整體服務中的位置。</p></div><div class="problem-grid">${problemCards}</div><div class="journey"><h3>使用者價值路徑</h3><div class="flow-row">${flow}</div></div><div class="sdd-note"><strong>SDD 設計依據</strong>${feature.sdd.map((item) => `<p>${esc(item)}</p>`).join("")}</div></section>
