...
import { alertItems } from "@/data/demo";
...
export default function AlertsPage() {
  ...
  const handleAcknowledge = (item: AlertItem) => {
  ...
  const handleDestination = () => {
    if (!selectedAlert) return;
    const visual = getAlertVisual(selectedAlert.kind);
    setReadIds((current) => new Set(current).add(selectedAlert.id));
    onClose();
    navigate(visual.destination);
  };
