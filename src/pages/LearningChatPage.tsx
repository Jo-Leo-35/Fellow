...
import {
  answerLearningQuestion, getLearningScenario, learningScenarios, materialChunks,
  resolveLearningScenario, retrieveMaterials,
  type LearningScenario, type LearningStep, type MaterialChunk,
} from "@/data/learningScenarios";

type LearningReply = ReturnType<typeof answerLearningQuestion>;
interface Exchange { id: number; question: string; time: string; reply: LearningReply }
...
function AnswerCard({ scenario, reply, onOpen }: { scenario: LearningScenario; reply: LearningReply; onOpen: (view: SourceView) => void }) {
  ...
  return (
    <Box as="article" aria-labelledby="learning-answer-title" bg="white" border="1px solid #DFEAEE" borderRadius="18px" overflow="hidden" boxShadow="0 8px 26px rgba(20,50,74,.05)">
      <VStack align="stretch" spacing="18px" p={{ base: "17px", sm: "20px" }}>
        ...
        <Text fontSize="14px" lineHeight="1.85" color="#3E5668">{scenario.summary}</Text>
        <Box bg="#F1F7FE" border="1px solid #E2EDFB" borderRadius="12px" p="13px" textAlign="center">
        ...
        <Box id="learning-animation" tabIndex={-1} scrollMarginTop="12px" borderRadius="16px" _focusVisible={{ outline: "2px solid", outlineColor: "brand.400", outlineOffset: "3px" }}><ScienceSimulation topic={scenario.id} /></Box>
        ...
        <SimpleGrid columns={2} spacing="9px">
          <Button h="42px" bg="brand.500" color="white" _hover={{ bg: "brand.600" }} fontSize="13px" aria-expanded={extra === "practice"}
            onClick={() => { setExtra("practice"); setChoice(null); setShowAnswer(false); }}>我想試試</Button>
          <Button h="42px" variant="outline" borderColor="#C8D8E2" color="navy.700" fontSize="13px" aria-expanded={extra === "alternate"}
            onClick={() => setExtra("alternate")}>再解釋一次</Button>
        </SimpleGrid>
...
function MaterialsModal({ view, onClose }: { view: SourceView | null; onClose: () => void }) {
  ...
  const sources = view?.library && search.trim() ? retrieveMaterials(search) : view?.sources ?? [];
  return (
...
function LearningSession({ scenario, initialQuestion, initialImage }: { scenario: LearningScenario; initialQuestion: string; initialImage: string }) {
  ...
  const lastQuestion = initialQuestion || (initialImage ? "" : scenario.question);
  const initialReply = lastQuestion ? answerLearningQuestion(lastQuestion, scenario.id) : null;
  const showFullAnswer = initialReply?.kind === "answer" && initialReply.title === scenario.title;
  const context = messages.length ? messages[messages.length - 1].reply.topic : scenario.id;
  const followUpScenario = getLearningScenario(context) ?? scenario;
  ...
  const sendQuestion = (text: string) => {
    ...
    const imageNote = selectedImage ? `（已選擇圖片：${selectedImage.name}）` : "";
    const reply: LearningReply = question ? answerLearningQuestion(question, context) : {
      kind: "unsupported", title: "請把圖片中的問題打出來", summary: "目前無法讀取圖片中的文字。請輸入題目內容，我們就能一起查找相關教材。", steps: [], sources: [], topic: context,
    };
    setMessages((current) => [...current, { id: current.length + 1, question: [question, imageNote].filter(Boolean).join("\n"), time: new Intl.DateTimeFormat("zh-TW", { hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date()), reply }]);
  ...
  const handleImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && !file.type.startsWith("image/")) { setFeedback("請選擇圖片檔案。"); setSelectedImage(null); event.target.value = ""; return; }
  ...
  const composer = (
    <Box as="footer" flexShrink={0} px="12px" pt="10px" pb="max(12px, env(safe-area-inset-bottom))" bg="white" borderTop="1px solid #E6EEF1" zIndex={4}>
      ...
      <Flex as="form" onSubmit={handleSubmit} align="center" gap="4px" p="4px" bg="#F8FBFC" border="1px solid #DDE8EC" borderRadius="15px">
        <Input value={draft} onChange={(event) => { setDraft(event.target.value); if (feedback) setFeedback(""); }}
        ...
        <IconButton type="submit" aria-label="送出問題" icon={<SendHorizontal size={18} />} size="sm" flexShrink={0} borderRadius="full" bg="brand.500" color="white" _hover={{ bg: "brand.600" }} />
      </Flex>
      ...
      <VStack ref={topRef} align="stretch" spacing="16px" px="12px" pt="18px" pb="20px">
        <Box px="3px">
          <HStack justify="space-between" mb="9px">
            ...
            <Button size="xs" variant="ghost" color="brand.700" leftIcon={<BookOpenText size={14} />} onClick={() => setSourceView({ title: "教材庫", sources: materialChunks, library: true })}>教材庫</Button>
          </HStack>
        ...
        <UserBubble text={[lastQuestion, initialImage ? `（圖片：${initialImage}；請輸入題目文字）` : ""].filter(Boolean).join("\n")} />
        {initialReply ? showFullAnswer ? <AnswerCard scenario={scenario} reply={initialReply} onOpen={setSourceView} /> : <FollowUpAnswer reply={initialReply} onOpen={setSourceView} /> : <Text fontSize="13px" color="#526B7D">請把圖片中的題目輸入下方，或選擇上方主題開始學習。</Text>}
        <VStack as="section" role="log" aria-label="後續問答" aria-live="polite" align="stretch" spacing="14px">
          {messages.map((message) => <VStack key={message.id} align="stretch" spacing="12px"><UserBubble text={message.question} time={message.time} /><FollowUpAnswer reply={message.reply} onOpen={setSourceView} /></VStack>)}
        </VStack>
        <Box as="section" role="region" aria-label="建議追問">
          <Text mb="9px" px="3px" fontSize="12px" color="#657F90">你也可以接著問 · {followUpScenario.title}</Text>
          <VStack align="stretch" spacing="7px">{followUpScenario.followUps.map((item) => <Button key={item.question} variant="outline" bg="white" borderColor="#DDE8ED" color="navy.700" borderRadius="12px" minH="39px" h="auto" py="10px" px="12px" whiteSpace="normal" textAlign="left" fontSize="12px" lineHeight="1.7" justifyContent="space-between" rightIcon={<ChevronRight size={14} />} onClick={() => sendQuestion(item.question)}>{item.question}</Button>)}</VStack>
        </Box>
...
export default function LearningChatPage() {
  ...
  const question = (params.get("q") ?? params.get("history") ?? "").trim();
