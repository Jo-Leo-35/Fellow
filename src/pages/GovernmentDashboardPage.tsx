...
export default function GovernmentDashboardPage() {
  ...
  const data = useMemo(
    () => getGovernmentDashboard(period, region),
    [period, region],
  );
  const detailData = useMemo(
    () =>
      detail
        ? getGovernmentDashboard(period, detail.region, detail.topic)
        : null,
    [period, detail],
  );
  ...
  const exportCsv = () => {
    const blob = new Blob([governmentCsv(period, region)], {
        ...
        <HStack align="flex-start" flex="1" spacing="14px">
          ...
          <Box>
            ...
            <Text color="navy.700" fontSize="13px" lineHeight="1.9">
              {regionLabel(region)}的「{insight.label}」需求
              {insight.needs >= insight.previous.needs ? "增加" : "減少"}{" "}
              {Math.abs(
                percentageChange(insight.needs, insight.previous.needs),
              ).toFixed(1)}
              %，本期共 {number(insight.needs)} 件；其中{" "}
              {number(insight.potential)} 件待關注。
            </Text>
            <Text color="gray.500" fontSize="12px" lineHeight="1.8" mt="3px">
              建議檢視相關資源的說明與可取得性，協助需求順利銜接。
            </Text>
          </Box>
        </HStack>
        <Button
            ...
            <Surface p="22px">
              ...
              {tracking.length ? (
                <VStack align="stretch" spacing="10px">
                  {tracking.map((item) => {
                    const stats = getGovernmentDashboard(
                      period,
                      item.region,
                      item.topic,
                    );
                    ...
                    return (
                      ...
                      >
                        <Box flex="1" minW="160px">
                          ...
                          <Text fontSize="12px" color="gray.500" mt="5px">
                            {periodLabels[period]} ·{" "}
                            {number(stats.totals.needs)} 件資源需求
                          </Text>
                        </Box>
                        <Button
                          size="xs"
