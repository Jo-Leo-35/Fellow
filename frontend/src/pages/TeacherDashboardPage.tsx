...
import { emptyTeacherCounts, type StudentSummary } from "@/api/dashboardPresentation";
import type { AttentionThresholdWire } from "@/types/wire";
...
function AnimationLink({
  ...
  return (
    <Button
      ...
      as={RouterLink}
      to={"/learning-chat.html?topic=" + topic.id}
    >
      {label}
    </Button>
  );
...
export default function TeacherDashboardPage() {
  ...
  const snapshotQuery=useQuery({queryKey:['teacher',filters,settings.attentionThreshold],queryFn:({signal})=>dashboardApi.teacher({...filters,subject:filters.subject,attentionThreshold:settings.attentionThreshold as AttentionThresholdWire},{signal})});
  const snapshot=snapshotQuery.data;const summary=snapshot?.summary??emptyTeacherCounts;const previousSummary=snapshot?.previousSummary??emptyTeacherCounts;
  const snapshotLabel=snapshot?.asOf ? new Date(snapshot.asOf).toLocaleDateString('zh-TW') : '';

  const topics=(snapshot?.topics??[]).map(item=>({id:item.topic,title:item.title,subject:item.subject,misconception:item.misconception,activity:item.suggestedActivity,question:item.suggestedQuestion,duration:item.durationMinutes,questions:item.questionCount,practiceCount:item.practiceCount,correctCount:item.correctCount,gapCount:item.gapCount,studentCount:item.studentCount,accuracy:item.accuracyPercentage}));
  const students:StudentSummary[]=(snapshot?.roster??[]).map(item=>({...item,id:item.studentId,accuracy:item.accuracyPercentage,animationCount:item.animationCompletedCount,mainTopic:topics.find(topic=>topic.id===item.mainTopic)}));
  const teacherStudents=students;
  const attentionStudents=students.filter(item=>item.needsAttention).sort((a,b)=>(a.accuracy??100)-(b.accuracy??100));
  const activeStudents=students.filter(item=>item.questionCount>0);
  const selectedStudent=students.find(item=>item.id===selectedStudentId);
  const leadingTopic=topics[0];const correctCount=summary.correctCount;const accuracy=summary.accuracyPercentage;const animationCount=summary.animationCompletedCount;const gapCount=summary.gapCount;
  ...
  const trendChart = useMemo<EChartsOption>(() => {
    const buckets=(snapshot?.trend??[]).map(item=>({label:item.label,total:item.questionCount,missed:item.gapCount}));
    return {
      animation: !window.matchMedia("(prefers-reduced-motion: reduce)").matches,
      animationDuration: 500,
      textStyle: { fontFamily: "Noto Sans TC, sans-serif" },
  ...
  const metrics = (
    <SimpleGrid columns={{ base: 1, sm: 3 }} spacing="16px" mb="20px">
      <Metric
        ...
        previous={previousSummary.questionCount}
      ...
      <Metric
        ...
        previous={
          previousSummary.activeStudentCount
        }
        icon={Users}
        color="#0EAD9C"
      ...
      <Metric
        ...
        previous={
          previousSummary.attentionCount
        }
        icon={LifeBuoy}
        color="#EF8736"
  ...
  const gapPanel = (
    <Surface p={{ base: "18px", md: "22px" }} minW={0}>
      <Heading
        title="最常遇到的學習困難"
        description="依理解卡點訊號次數排序，找到值得一起講解的概念。"
        action={
          activeView === "overview" ? (
            <Button
              variant="link"
  ...
  if(!snapshot) return <DashboardShell edition="教師版" title="教師工作台" ownerName="" ownerDetail="" items={navigation}><ApiState loading={snapshotQuery.isPending} error={snapshotQuery.error} retry={()=>void snapshotQuery.refetch()}/></DashboardShell>;
  return (
    <DashboardShell
      edition="教師版"
    ...
    >
          ...
          <Grid templateColumns={{ base: "1fr", xl: "1.15fr 1fr" }} gap="20px">
            <Surface p="22px" minW={0}>
              <Heading
                title="學習參與趨勢"
                description={
                  periodLabels[filters.period] + "的提問與需鞏固訊號"
                }
              />
              <ReactECharts
                option={trendChart}
              ...
              <VStack align="stretch" spacing="19px">
                <Box>
                  ...
                  <Text fontSize="11px" color="gray.500" mt="7px">
                    {correctCount} / {summary.practiceCount} 次已記錄作答答對
                  </Text>
                </Box>
                <Box>
                  <Flex justify="space-between" mb="8px">
                    ...
                    <Text fontWeight="800" fontSize="14px" color="blue.600">
                      {summary.animationCompletionPercentage!==null ? summary.animationCompletionPercentage + "%" : "未知"}
                    </Text>
                  </Flex>
                  <Progress
                    value={
                      summary.animationCompletionPercentage??0
                    }
                    size="sm"
                    borderRadius="full"
                    colorScheme="blue"
                  ...
                  />
                  <Text fontSize="11px" color="gray.500" mt="7px">
                    {animationCount} / {summary.animationObservationCount} 次動畫觀察完成觀看
                  </Text>
                </Box>
                <Divider />
                <Button
                  ...
                  <HStack spacing="11px">
                    ...
                    <Box>
                      ...
                      <Text fontSize="11px" color="gray.500" mt="4px">
                        ...
                        正確率 {topic.accuracy===null?"未知":`${topic.accuracy}%`}
                      </Text>
                    </Box>
                  </HStack>
                  <Badge
            ...
            >
              <HStack mb="16px">
                ...
                <Text fontWeight="800" color="navy.900">
                  {snapshot.authorizedScope.label}
                </Text>
              </HStack>
              <Text fontSize="13px" lineHeight="2" color="gray.600">
                目前授權範圍包含 {snapshot.authorizedScope.classIds.length} 個班級，共 {summary.rosterStudentCount}
      ...
      {activeView !== "settings" && (
        <Flex
          ...
          fontSize="10px"
        >
          <Text>{snapshot.authorizedScope.label}</Text>
          <Text>
                ...
                <Heading
                  ...
                  description={
                    selectedStudent.needsAttention
                      ...
                      : selectedStudent.practiceCount < 3
                        ? "目前練習較少，可以先累積幾次概念觀察。"
                        : "持續觀察不同概念的練習，再調整教學節奏。"
                  }
                />
                <VStack spacing="13px" align="stretch">
                  {selectedStudent.topicSummaries.map((topic) => (
                    <Box key={topic.topic}>
                      <Flex
                        justify="space-between"
                        align="baseline"
                      ...
                      <Progress
                        value={topic.accuracyPercentage ?? 0}
                        size="xs"
                        borderRadius="full"
                        colorScheme={
                          topic.accuracyPercentage !== null &&
                          topic.accuracyPercentage < settings.attentionThreshold
                            ? "orange"
                            : "teal"
                        }
                      />
